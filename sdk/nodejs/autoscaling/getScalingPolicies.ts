// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of scaling policies
 */
export function getScalingPolicies(args: GetScalingPoliciesArgs, opts?: pulumi.InvokeOptions): Promise<GetScalingPoliciesResult> {

    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("volcengine:autoscaling/getScalingPolicies:getScalingPolicies", {
        "ids": args.ids,
        "nameRegex": args.nameRegex,
        "outputFile": args.outputFile,
        "scalingGroupId": args.scalingGroupId,
        "scalingPolicyNames": args.scalingPolicyNames,
        "scalingPolicyType": args.scalingPolicyType,
    }, opts);
}

/**
 * A collection of arguments for invoking getScalingPolicies.
 */
export interface GetScalingPoliciesArgs {
    /**
     * A list of scaling policy ids.
     */
    ids?: string[];
    /**
     * A Name Regex of scaling policy.
     */
    nameRegex?: string;
    /**
     * File name where to save data source results.
     */
    outputFile?: string;
    /**
     * An id of the scaling group to which the scaling policy belongs.
     */
    scalingGroupId: string;
    /**
     * A list of scaling policy names.
     */
    scalingPolicyNames?: string[];
    /**
     * A type of scaling policy. Valid values: Scheduled, Recurrence, Manual, Alarm.
     */
    scalingPolicyType?: string;
}

/**
 * A collection of values returned by getScalingPolicies.
 */
export interface GetScalingPoliciesResult {
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly ids?: string[];
    readonly nameRegex?: string;
    readonly outputFile?: string;
    /**
     * The id of the scaling group to which the scaling policy belongs.
     */
    readonly scalingGroupId: string;
    /**
     * The collection of scaling policy query.
     */
    readonly scalingPolicies: outputs.autoscaling.GetScalingPoliciesScalingPolicy[];
    readonly scalingPolicyNames?: string[];
    /**
     * The type of the scaling policy.
     */
    readonly scalingPolicyType?: string;
    /**
     * The total count of scaling policy query.
     */
    readonly totalCount: number;
}
/**
 * Use this data source to query detailed information of scaling policies
 */
export function getScalingPoliciesOutput(args: GetScalingPoliciesOutputArgs, opts?: pulumi.InvokeOptions): pulumi.Output<GetScalingPoliciesResult> {
    return pulumi.output(args).apply((a: any) => getScalingPolicies(a, opts))
}

/**
 * A collection of arguments for invoking getScalingPolicies.
 */
export interface GetScalingPoliciesOutputArgs {
    /**
     * A list of scaling policy ids.
     */
    ids?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * A Name Regex of scaling policy.
     */
    nameRegex?: pulumi.Input<string>;
    /**
     * File name where to save data source results.
     */
    outputFile?: pulumi.Input<string>;
    /**
     * An id of the scaling group to which the scaling policy belongs.
     */
    scalingGroupId: pulumi.Input<string>;
    /**
     * A list of scaling policy names.
     */
    scalingPolicyNames?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * A type of scaling policy. Valid values: Scheduled, Recurrence, Manual, Alarm.
     */
    scalingPolicyType?: pulumi.Input<string>;
}
